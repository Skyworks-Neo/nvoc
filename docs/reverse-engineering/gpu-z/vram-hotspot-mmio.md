...
> **Archived research:** Offsets, decode formulas, and access permissions are
> architecture-, board-, firmware-, and platform-specific. This document does
> not define a supported NVOC telemetry path. See the
> [archive policy](../README.md) before reproducing any direct-MMIO work.

Companion to [`per-rail-power.md`](per-rail-power.md). That doc covers the **Windows**
...
Sources: the `gddr6.c` sample (author's comments), [`per-rail-power.md`](per-rail-power.md),
