...
> **归档快照：** 本文记录特定 GPU-Z/NVIDIA 驱动组合上的调查流程。
> 地址、候选 ID 和模块布局不可直接套用到其他版本。当前项目结论与安全边界见
> [逆向工程归档索引](../README.md)。
...
1. **唯一钩点**：`nvapi_QueryInterface(uint32 id)` —— 所有 nvapi*.dll 都导出该符号（32/64 位同名）。
   ...
   - 真正的分发表 `off_1804DE000`：每条 16 字节 `[u64 func_ptr][u64 id]`，共 **2337** 条；
     其中 **1487 条** 不在当时的 `nvid.rs`（857 条）里。完整快照见
     [`../nvapi/evidence/query-interface-table-r610-74.txt`](../nvapi/evidence/query-interface-table-r610-74.txt)；
     `unknown_ids.txt` 是本地派生表，未归档。
...
3. **已实测排除的陷阱（重要）**：
   ...
   - 教训：**结构相似 ≠ 语义相同**。当时本地候选表中的 25 个 ID，任何静态挑出的，
...
...
```
`handler` 的低位（`…238c0` / `…235f10`）可直接和本地生成候选表的 `func_va` 低位比对。
...
- [`../nvapi/evidence/query-interface-table-r610-74.txt`](../nvapi/evidence/query-interface-table-r610-74.txt)：
  已归档的静态 ID 表；`unknown_ids.txt` 与候选交集表是可重新生成的本地产物，不提交。
