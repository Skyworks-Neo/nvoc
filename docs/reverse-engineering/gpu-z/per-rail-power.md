...
> **归档快照：** 本文保留调查过程、被推翻的假设与特定驱动上的测量。
> 后续 PowerMonitor 探针表明部分系统可能存在独立的 NVAPI 描述符/状态路径；
> 这不改变 GPU-Z 路径的静态分析，也不构成可移植的 NVOC 产品接口。
> 当前边界见 [逆向工程归档索引](../README.md)。
...
| 96 个已知 NVAPI 功耗/电压 ID 在测试笔记本上全空 | 该机型未通过这些已知接口提供分路数据 |
...
自己的驱动 + 每型号寄存器表的原因；其上游实现可作为该路线的公开参考。
...
详见 [`query-interface-tracing.md`](query-interface-tracing.md) §3；候选 ID 的实测结论已合并如下：
...
- 当时测试的笔记本 GPU 上这些接口返回空/NotSupported，实际可用面窄；
...
| [`query-interface-tracing.md`](query-interface-tracing.md) | WinDbg 动态定位 ID 的完整流程 + `0x7457cab5` 排除记录 |
| [`../nvapi/evidence/query-interface-table-r610-74.txt`](../nvapi/evidence/query-interface-table-r610-74.txt) | nvapi64_impl 的 2337 条 QI 分发表静态导出 |
| `unknown_ids.txt` / `gpu_control_rm_unknown_ids.txt` | 可从完整表重新生成的本地派生候选表（未提交） |
| [`trace-query-interface.js`](trace-query-interface.js) | Frida 双相 ID 捕获脚本（原 watts 假设已作废，保留作其他 ID 调查参考） |
| LibreHardwareMonitor / OpenHardwareMonitor 上游 | 同路线开源实现，可借鉴驱动与寄存器映射设计 |
...
调查期间的个人 memory 文件不属于可复现证据，未纳入仓库。可维护结论、负面结果、
候选 `0x7457cab5` 的排除记录和本机接口返回情况均已合并进本文；原始二进制与动态日志
按 [归档策略](../README.md) 保持在仓库外。
