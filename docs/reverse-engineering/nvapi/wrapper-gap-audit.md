...
> **Snapshot:** Counts and wrap status reflect the `nvapi-rs` tree on the date
> above. This report is preserved as audit evidence, not as a current backlog.
...
|---|---|---|---|
| 0xB4C5D8BA | `NvAPI_GPU_ClientQboostGetInfo` | unwrapped, **intentionally** | ruled out by the earlier PPAB-slider experiment |
...
is correctly documented by the corresponding GPUMonCmd audit evidence.
