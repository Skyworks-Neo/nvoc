...
2. The GUI auto-detects `nvoc-auto-optimizer.exe` in `../auto-optimizer/target/release/`
...
- **`src/cli_runner.py`**
  - Purpose: Wraps command-line interaction with the external nvoc-auto-optimizer CLI, handling command construction, process management, and result parsing.
...
- **Layered structure**
  ...
  - External dependency layer: nvoc-auto-optimizer CLI tool
...
2. GUI 会自动检测 `../auto-optimizer/target/release/` 下的 `nvoc-auto-optimizer.exe`
...
- **`src/cli_runner.py`**
  - 作用：封装与外部 nvoc-auto-optimizer CLI 工具的命令行交互，负责命令构建、进程管理和结果解析。
...
- **分层结构**
  ...
  - 外部依赖层：nvoc-auto-optimizer CLI 工具
