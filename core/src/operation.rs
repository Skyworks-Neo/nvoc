...
/// Query the QBoost / Dynamic-Boost controller (NDA 0xB4C5D8BA): active
/// controller index + current target power. The PPAB-paired power slider.
#[derive(Clone, Debug, Default)]
pub struct QueryNvapiQboostPower;

/// QBoost controller state result (power in **watts**).
#[derive(Clone, Debug)]
pub struct QboostPowerInfo {
    pub controller_index: usize,
    pub power_watt: Option<f64>,
}

impl GpuOperation for QueryNvapiQboostPower {
    type Output = Option<QboostPowerInfo>;

    fn kind(&self) -> OperationKind {
        OperationKind::QueryNvapiQboostPower
    }
    ...
    fn run(&self, target: &GpuTarget<'_>) -> Result<Self::Output, Error> {
        Ok(target
            ...
            .qboost_info()
            .map_err(Error::from)?
            .map(|r| QboostPowerInfo {
                controller_index: r.controller_index,
                power_watt: r.power_mw.map(|mw| mw as f64 / 1000.0),
            }))
    }
}

/// Set the QBoost / Dynamic-Boost controller target power in watts (NDA
/// 0xB78734AB, the PPAB-paired power slider). `controller_index` None ⇒ query.
#[derive(Clone, Debug)]
pub struct SetNvapiQboostPower {
    pub watts: u32,
    pub controller_index: Option<usize>,
}

impl GpuOperation for SetNvapiQboostPower {
    type Output = (usize, u32);

    fn kind(&self) -> OperationKind {
        OperationKind::SetNvapiQboostPower
    }

    fn run(&self, target: &GpuTarget<'_>) -> Result<Self::Output, Error> {
        target
            .nvapi()?
            .set_qboost_power(self.watts, self.controller_index)
            .map_err(Error::from)
    }
}

/// Reset the QBoost / Dynamic-Boost controller target power (NDA 0xB78734AB).
/// Writes 0xFFFFFFFF (reset) into the active controller entry.
#[derive(Clone, Debug, Default)]
pub struct ResetNvapiQboostPower {
    pub controller_index: Option<usize>,
}

impl GpuOperation for ResetNvapiQboostPower {
    type Output = ();

    fn kind(&self) -> OperationKind {
        OperationKind::ResetNvapiQboostPower
    }

    fn run(&self, target: &GpuTarget<'_>) -> Result<Self::Output, Error> {
        // 0xFFFFFFFF = reset sentinel (GPUMon writes -1 into the power dword).
        target
            .nvapi()?
            .set_qboost_power(0xFFFFFFFF, self.controller_index)
            .map(|_| ())
            .map_err(Error::from)
    }
}
