...
> **Snapshot:** Export counts and binding coverage apply to the DLL, public
> header, and Rust dependency versions recorded below. Re-run the comparison
> before treating any listed symbol as a current gap.
...
The locally captured `nvml.dll` used for the export comparison is a **2024-05**
build (driver R550+, ~NVML v12). How many NVML
...
1. Dumped all exports of the captured `nvml.dll` via `objdump -p` → **417** unique
   `nvml*` symbols.
...
Local data artifacts: the captured `nvml.dll` and CUDA 12.8 `nvml.h`. They are
not committed. Analysis is reproducible from `objdump -p nvml.dll` plus the
bindings.rs/header diffs.
...
a later `nvml.dll` from the R610 `r610_45` driver branch in IDA. This appendix
therefore has a different binary snapshot from the export-count comparison
above.
