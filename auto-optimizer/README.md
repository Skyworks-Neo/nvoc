...
- [兼容性一览（接口 × GPU 世代 / 基础功能）](#compatibility-interfaces-gpu-generations)
- [依赖与环境要求](#依赖与环境要求)
- [目录结构](#目录结构)
- [快速开始](#快速开始)
- [命令参考](#命令参考)
...
后端/功能兼容性矩阵改由 [NVOC-CLI](../cli/README.md#compatibility-overview-interface--gpu-generation--basic-functions) 维护，因为这些条目对应的是 `nvoc-cli` 暴露的 NVAPI/NVML 控制能力。

autoscan 专用的 GPU 世代支持请见[支持的 GPU 世代](#支持的-gpu-世代)。
...
| CUDA 运行库                                                | 内置 `cli-stressor-cuda-rs` worker 需要；优化器会将其作为隔离子进程启动。                                                                    |
| Administrator权限/Sudo权限                                | 超频参数写入接口需要管理权限，而大部分读取不需要                                                                                                    |

### 集成工作流

运行 `nvoc-auto-optimizer optimize --gpu <id>`。优化器会自行完成工作目录创建、重置、
初始导出、可恢复 autoscan、结果修正/导入以及最终导出。可用 `--mode ultrafast` 或
`--mode legacy` 切换模式，`--fresh` 丢弃已有恢复状态。

### 编译依赖（仅从源码构建时需要）

- Rust 工具链
- 目标架构对应构建工具
- 默认 `stressor-bundled` feature 会把 CUDA/Vulkan stressor 编入优化器。
- 可选 `stressor-external` fallback 会用结构化参数启动独立的
  `cli-stressor-cuda-rs`，不经过 shell wrapper。
...
传入参数 `1` 将清空 `ws\vfp.jsonl` 和 `ws\vfp-tem.csv`。
...
| `export-vfp-log [OPTIONS]` | 从 autoscan JSONL 日志解析并导出 VFP 点。 |
...
| `--test-exe <路径>` | `-w` | `cli-stressor-cuda-rs`（仅 external feature） | 外部 stressor 可执行文件路径；默认 build 使用内置子进程 |
| `--log <路径>` | `-l` | `./ws/vfp.jsonl`    | 结构化扫描日志路径                                                  |
| `-q <序列>`     | —    | `-`                 | 自定义扫描点序列（`-` 为自动）                                           |
...
参数与 `autoscan` 基本相同，但不支持 `--ultrafast`、`-m`（显存扫描）、`-q`（点序列）、`-o`（临时结果 CSV）及 `-i`（初始曲线），因为 Legacy 模式只有单一全局偏移。`-w`、`-l` 和 `-t` 仍可用于调整 CLI 压力测试封装脚本、结构化扫描日志和压力测试时长/重试循环。
...
| `-l <路径>`     | —    | `./ws/vfp.jsonl`                              | 结构化扫描日志（用于读取 ultrafast 关键点）                |
...
`nvoc-auto-optimizer optimize` 在调用 `autoscan-vfp` 前会依次执行：
...
结果以结构化 `voltage_range` 事件记录到 `vfp.jsonl` 中，断点续扫时直接从 JSONL 日志读取，跳过此阶段。
...
每个电压点扫描完成后，结果实时追加到 `vfp-tem.csv`，并记录到 `vfp.jsonl`（支持断点续扫）。
...
`nvoc-auto-optimizer optimize` 在 `autoscan` 完成后自动调用：
...
每个电压点扫描完成后，结构化结果会实时追加到 `ws\vfp.jsonl`。下次运行时，工具会解析日志中的：

- `voltage_range`：跳过电压范围探测阶段
- `point_finished`：从该点的下一个点继续
- `test_result`：直接恢复二分搜索的收敛状态
- `scan_mode` / `key_points`：恢复普通或 ultrafast 模式状态

如果 JSONL 文件中存在损坏记录，工具会打印解析错误，尽可能从有效记录恢复，并在恢复断点续扫前要求确认。

因此，无论是正常退出、手动中断还是崩溃重启，**只需再次运行 `nvoc-auto-optimizer optimize`，扫描即可从中断处继续**，无需重新开始。

若需从头开始，运行 `nvoc-auto-optimizer optimize --fresh` 清除日志。
...
| 文件                 | 用途            | 说明                            |
|--------------------|---------------|-------------------------------|
| `ws\vfp.jsonl`     | 结构化扫描日志      | 断点续扫的核心依据；删除此文件会导致从头开始        |
...
稳定极限。工具会自动退回并从更保守的值继续。若系统卡死超过 3 分钟，请手动强制重启，然后重新运行 `nvoc-auto-optimizer optimize`
